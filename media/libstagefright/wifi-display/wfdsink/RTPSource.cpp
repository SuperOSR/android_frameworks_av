...
struct RTPSource::Source : public RefBase {
...
void RTPSource::Source::initSeq(uint16_t seq) {
    mMaxSeq = seq;
    mCycles = 0;
    mBaseSeq = seq;
...
bool RTPSource::Source::updateSeq(uint16_t seq, const sp<ABuffer> &buffer) {
...
void RTPSource::Source::queuePacket(const sp<ABuffer> &buffer) {
    sp<AMessage> msg = mQueueBufferMsg->dup();
    msg->setBuffer("buffer", buffer);
    msg->post();
}

void RTPSource::Source::addReportBlock(
...
status_t RTPSource::init(bool useTCPInterleaving) {
...
//dequeue
sp<RTPJitterBuffer> RTPSource::getRTPJitterBuffer() {
    return mRTPJitterBuffer;
}

int32_t RTPSource::getRTPPort() const {
    return mRTPPort;
}

void RTPSource::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        case kWhatRTPNotify:
...
status_t RTPSource::injectPacket(bool isRTP, const sp<ABuffer> &buffer) {
...
status_t RTPSource::parseRTP(const sp<ABuffer> &buffer) {
    size_t size = buffer->size();
    if (size < 12) {
        // Too short to be a valid RTP header.
        ALOGE("1.ERROR_MALFORMED");
    ...
    if (index < 0) {
        if (mRTPJitterBuffer == NULL) {
            sp<AMessage> notifyLost = new AMessage(kWhatPacketLost, id());
            notifyLost->setInt32("ssrc", srcId);

            mRTPJitterBuffer = new RTPJitterBuffer(notifyLost);
            looper()->registerHandler(mRTPJitterBuffer);
            //add rtpbuffer handler for local looper, 
            //rtpbuffer can receive msg from rtpsource
            //notify,wake up thread,callback; 
        }

        sp<AMessage> queueBufferMsg =
            new AMessage(RTPJitterBuffer::kWhatQueueBuffer, mRTPJitterBuffer->id());

        ALOGD("1.seqNo(%d)", seqNo);
...
status_t RTPSource::parseRTCP(const sp<ABuffer> &buffer) {
    const uint8_t *data = buffer->data();
    size_t size = buffer->size();
...
status_t RTPSource::parseBYE(const uint8_t *data, size_t size) {
...
status_t RTPSource::parseSR(const uint8_t *data, size_t size) {
...
void RTPSource::scheduleSendRR() {
    (new AMessage(kWhatSendRR, id()))->post(2000000ll);
}

void RTPSource::addSDES(const sp<ABuffer> &buffer) {
...
void RTPSource::onPacketLost(const sp<AMessage> &msg) {
    uint32_t srcId;
