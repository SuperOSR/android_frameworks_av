...
status_t CedarXMetadataRetriever::setDataSource(int fd, int64_t offset, int64_t length, const char *file_path)
{
	LOGV("setDataSource(%d, %lld, %lld, %p)", fd, offset, length, file_path);
    fd = dup(fd);
    ...
    cdx_ext_fd.offset = offset;
    cdx_ext_fd.length = length;
    if(mRetriever->control(mRetriever, CDX_SET_DATASOURCE_FD, (unsigned int)(&cdx_ext_fd), (unsigned int)file_path) != 0)
    	return UNKNOWN_ERROR;
