...
status_t CedarXPlayer::setDataSource(int fd, int64_t offset, int64_t length, const char *file_path)
{
	//Mutex::Autolock autoLock(mLock);
	LOGV("CedarXPlayer::setDataSource fd with file name.");
	CedarXExternFdDesc ext_fd_desc;
	ext_fd_desc.fd = fd;
	ext_fd_desc.offset = offset;
	ext_fd_desc.length = length;
	...
	mPlayer->control(mPlayer, CDX_SET_DATASOURCE_FD, (unsigned int)&ext_fd_desc, (unsigned int)file_path);
	return OK;
}
...
status_t CedarXPlayer::play_l(int command)
{
    ...
    int switchsub_flag = 0;
    int switchaudio_flag = 0;
...
//                    || TEST_FORCE_GPU_RENDER)
    ...
    if(switchsub_flag)
    {
        LOGD("switch subtitle index=%d", mSubtitleParameter.mSubtitleIndex);
        mPlayer->control(mPlayer, CDX_CMD_SWITCHSUB, mSubtitleParameter.mSubtitleIndex, 0);
    }
    if(switchaudio_flag)
    {
        LOGD("switch audio index=%d", mAudioTrackIndex);
	    mPlayer->control(mPlayer, CDX_CMD_SWITCHTRACK, mAudioTrackIndex, 0);
    }
	mSeeking = false;
	mTagPlay = 0;
